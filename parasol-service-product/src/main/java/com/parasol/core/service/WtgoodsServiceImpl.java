...
public class WtgoodsServiceImpl implements WtgoodsService {
    ...
    @Override
    public List selectCat() {
        List<HashMap> list1 = wtgoodsMapper.selectFirst();
        List<HashMap> list2 = wtgoodsMapper.selectSec();
        List<HashMap> list3 = wtgoodsMapper.selectThird();
        List array = new ArrayList();
        for (Map map : list1) {
            Map map1 = new HashMap();
            Object cat_sid1 = map.get("cat_sid");
            Object cat_name1 = map.get("cat_name");
            List secList = new ArrayList();
            for (Map map2 : list2) {
                Object cat_patentid =  map2.get("cat_patentid");
                if (cat_patentid.equals(cat_sid1) ) {
                    Map map3 = new HashMap();
                    Object cat_sid2 =  map2.get("cat_sid");
                    Object cat_name =  map2.get("cat_name");
                    List thirdlist = new ArrayList();
                    for (Map map4 : list3) {
                        Object cat_id3 =  map4.get("cat_id");
                        if (cat_sid2.equals(cat_id3)) {
                            Map map5 = new HashMap();
                            Object cat_sid3 =  map4.get("cat_sid");
                            Object cat_name3 =  map4.get("cat_name");
                            map5.put("value", cat_sid3);
                            map5.put("label", cat_name3);
                            thirdlist.add(map5);
                        }
                    }
                    map3.put("value", cat_sid2);
                    map3.put("label", cat_name);
                    map3.put("children", thirdlist);
                    secList.add(map3);
                }
            }
            map1.put("value", cat_sid1);
            map1.put("label", cat_name1);
